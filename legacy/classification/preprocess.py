"""LEGACY — classification preprocessing (superseded by ``regression.preprocess``).

Kept for reference only. Writes chunked CSVs with a binary ``target`` column
produced by ``alt_label_df`` instead of the regression high/low targets.

This module used to run on import, silently overwriting ``data/split_finer_data``.
It is now behind a main guard: run it deliberately with
``python -m legacy.classification.preprocess``.
"""
