...
class AdvancedTrainingPipeline:
    """Advanced training pipeline for recent data and instrument-specific fine-tuning"""
    
    def __init__(self, base_model_path='checkpoints/best_model.keras'):
        self.base_model_path = base_model_path
        self.recent_data_path = 'data/split_finer_data'
        self.target_instruments = ['EURUSD#', 'GBPUSD#', 'USDJPY#', 'AUDUSD#']
        
        # Initialize managers for each stage
        ...
        # Initialize instrument-specific managers
        self.instrument_managers = {}
        for instrument in self.target_instruments:
            self.instrument_managers[instrument] = TrainingStateManager(f'checkpoints_{instrument}')
    ...
    def setup_model_for_training(self, base_model, learning_rate_factor=0.1):
        ...
        lr_schedule = WarmupCosineDecay(
            ...
            warmup_steps=WARMUP // 2,  # Shorter warmup for fine-tuning
            decay_steps=DECAY // 2     # Shorter decay for fine-tuning
        )
    ...
    def train_on_recent_data(self):
        """Stage 1: Train on recent data (data/split_finer_data) with all instruments"""
        print("\n" + "="*80)
        print("STAGE 1: TRAINING ON RECENT DATA (split_finer_data)")
        print("="*80)
        ...
        if self.recent_training_manager.has_checkpoint():
            ...
            try:
                ...
                (train_dataset, val_dataset, test_dataset), (train_steps, val_steps, test_steps) = get_datasets_and_steps(instruments, base_path=self.recent_data_path)
    ...
    def fine_tune_instrument_specific(self, recent_best_model, instrument):
        """Stage 2: Fine-tune for specific instrument"""
        print(f"\n" + "="*80)
        print(f"STAGE 2: FINE-TUNING FOR {instrument}")
        print("="*80)
        
        manager = self.instrument_managers[instrument]
        
        # Check if we should resume or start fresh for this instrument
        if manager.has_checkpoint():
            print(f"Found existing {instrument} checkpoint - resuming...")
            try:
                model, optimizer_state, current_epoch, global_step, best_val_loss, patience_count, history, epoch_durations = manager.load_training_state()
                
                # Load instrument-specific dataset
                (train_dataset, val_dataset, test_dataset), (train_steps, val_steps, test_steps) = get_datasets_and_steps([instrument], base_path=self.recent_data_path)
                
                # Setup optimizer
                _, optimizer = self.setup_model_for_training(model, learning_rate_factor=0.01)  # Even lower LR for fine-tuning
                restore_optimizer_state(optimizer, optimizer_state, model, global_step)
                
                initial_epoch = current_epoch
                print(f"Resumed {instrument} training from epoch {current_epoch}")
                
            except Exception as e:
                print(f"Failed to resume {instrument} training: {e}")
                print(f"Starting fresh {instrument} training...")
                model = tf.keras.models.clone_model(recent_best_model)
                model.set_weights(recent_best_model.get_weights())
                model, optimizer = self.setup_model_for_training(model, learning_rate_factor=0.01)
            ...
            print(f"Starting fresh {instrument} fine-tuning...")
            model = tf.keras.models.clone_model(recent_best_model)
            model.set_weights(recent_best_model.get_weights())
            model, optimizer = self.setup_model_for_training(model, learning_rate_factor=0.01)
            
            # Load instrument-specific dataset
            print(f"Loading {instrument} data from {self.recent_data_path}")
            (train_dataset, val_dataset, test_dataset), (train_steps, val_steps, test_steps) = get_datasets_and_steps([instrument], base_path=self.recent_data_path)
        ...
        # Check if we can start an epoch
        if not self.time_manager.can_start_epoch():
            print(f"Cannot start {instrument} training - insufficient time remaining")
            return model
        
        # Print training status
        print(f"\n{instrument} FINE-TUNING STATUS:")
        print(f"Starting from epoch: {initial_epoch}")
        print(f"Global step: {global_step}")
        print(f"Current best validation loss: {best_val_loss:.6f}")
        print(f"Training steps per epoch: {train_steps}")
        ...
        if remaining_epochs > 0:
            print(f"\nStarting {instrument} fine-tuning for up to {remaining_epochs} more epochs...")
            ...
            print(f"{instrument} fine-tuning completed!")
        ...
        best_instrument_path = os.path.join(manager.checkpoint_dir, 'best_model.keras')
        if os.path.exists(best_instrument_path):
            print(f"Loading best {instrument} model from: {best_instrument_path}")
            from transformer_builder import (
                LearnablePositionalEncoding, 
                StochasticGatedTransformerBlock, 
                AddTypeEmbedding, 
            ...
            print(f"No best {instrument} model found, returning current model")
            return model
    ...
    def run_complete_pipeline(self):
        ...
        print("1. Stage 1: Train on recent data (split_finer_data) with all instruments")
        print("2. Stage 2: Fine-tune individual models for each target instrument")
        print(f"   Target instruments: {self.target_instruments}")
        print("="*80)
        
        # Stage 1: Train on recent data
        ...
        # Stage 2: Fine-tune for each instrument
        instrument_models = {}
        for instrument in self.target_instruments:
            if not self.time_manager.can_start_epoch():
                print(f"Insufficient time remaining for {instrument} fine-tuning")
                break
                
            print(f"\nStarting fine-tuning for {instrument}...")
            instrument_model = self.fine_tune_instrument_specific(recent_best_model, instrument)
            instrument_models[instrument] = instrument_model
            
            print(f"{instrument} fine-tuning completed!")
            print(f"Best {instrument} model saved to: {self.instrument_managers[instrument].checkpoint_dir}/best_model.keras")
        ...
        print(f"1. Recent data model: {self.recent_training_manager.checkpoint_dir}/best_model.keras")
        
        for instrument in self.target_instruments:
            if instrument in instrument_models:
                print(f"2. {instrument} model: {self.instrument_managers[instrument].checkpoint_dir}/best_model.keras")
            else:
                print(f"2. {instrument} model: NOT COMPLETED (insufficient time)")
        
        print("\nAll models are clearly labeled and saved in separate directories.")
        print("Training can be resumed for any incomplete stage by running this script again.")
        print("="*80)
        
        return {
            'recent_model': recent_best_model,
...
def main():
    ...
    if results:
        ...
        print("You can now use the specialized models for trading on their respective instruments.")
    else:
