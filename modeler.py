...
def create_regression_model(input_shape=(64, 5), other_input_shape=(64, 5), 
    ...
    def create_partial_hour_context(minutes_val, length_val, embed_dim=8):
        ...
        minutes_sin = Lambda(lambda x: sin(2 * 3.14159 * x))(minutes_norm)
        minutes_cos = Lambda(lambda x: cos(2 * 3.14159 * x))(minutes_norm)
    ...
    partial_temporal_expanded = Lambda(lambda x: expand_dims(x, axis=1), 
    ...
    # 1. Hourly data first (longest-term context)
    # 2. Partial hour data second (current hour context, hourly-normalized)
    # 3. 5-minute data last (immediate high-resolution context)
    ...
    high_dense = Dropout(0.1)(high_dense)
    ...
    low_dense = Dropout(0.1)(low_dense)
