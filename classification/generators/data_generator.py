...
class InstrumentChunkManager:
    ...
    def get_chunk(self, chunk_idx: int) -> pl.DataFrame:
        ...
        required_cols = self.feature_columns + ['include', 'time', 'target']
...
class SingleInstrumentProcessor:
    ...
    def __init__(self, instrument_config: InstrumentConfig, config: MultiInstrumentDatasetConfig):
        ...
        self.valid_indices: List[Tuple[int, int]] = []  # (chunk_idx, row_idx)
        self.positive_indices: List[Tuple[int, int]] = []
        self.negative_indices: List[Tuple[int, int]] = []
        self._build_indices()
        ...
        for chunk_idx in range(self.chunk_manager.get_chunk_count()):
            ...
            for row_idx in range(self.config.lookback_window, chunk_df.shape[0]):
                if chunk_df[row_idx, 'include'] == 1:
                    ...
                    self.valid_indices.append(index_tuple)
                    if target_value == 0:
                        self.negative_indices.append(index_tuple)
                    else:
                        self.positive_indices.append(index_tuple)
        
        logger.info(f"Built indices for {self.instrument_config.name}: "
                   f"{len(self.positive_indices)} positive, {len(self.negative_indices)} negative samples")
    
    def extract_sample(self, chunk_idx: int, row_idx: int) -> Optional[Tuple[np.ndarray, np.ndarray, int]]:
        ...
        try:
            ...
            # Extract hourly sequence - THIS IS CRITICAL: using this instrument's hourly data
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'positive_samples': len(self.positive_indices),
...
class MultiInstrumentIndexManager:
    """Manages global indexing across all instruments with SMOTE balancing"""
    
    def __init__(self, processors: List[SingleInstrumentProcessor], config: MultiInstrumentDatasetConfig):
        self.processors = processors
        self.config = config
        # Global indices: (instrument_idx, chunk_idx, row_idx)
        self.global_positive_indices: List[Tuple[int, int, int]] = []
        self.global_negative_indices: List[Tuple[int, int, int]] = []
    ...
    def _build_global_indices(self):
        ...
        for instrument_idx, processor in enumerate(self.processors):
            # Add instrument index to all indices
            for chunk_idx, row_idx in processor.positive_indices:
                self.global_positive_indices.append((instrument_idx, chunk_idx, row_idx))
            
            for chunk_idx, row_idx in processor.negative_indices:
                self.global_negative_indices.append((instrument_idx, chunk_idx, row_idx))
        
        logger.info(f"Global indices built: {len(self.global_positive_indices)} positive, "
                   f"{len(self.global_negative_indices)} negative samples across {len(self.processors)} instruments")
        
        if len(self.global_positive_indices) + len(self.global_negative_indices) == 0:
            raise ValueError("No valid samples found across all instruments")
    
    def get_balanced_indices(self) -> List[Tuple[int, int, int]]:
        """Get indices with SMOTE balancing applied across all instruments"""
        if not self.config.apply_smote:
            return self.global_positive_indices + self.global_negative_indices
        
        if not self.global_positive_indices or not self.global_negative_indices:
            logger.warning("SMOTE requested but one class is empty across all instruments, returning all indices")
            return self.global_positive_indices + self.global_negative_indices
        
        # Balance classes efficiently across all instruments
        majority_size = max(len(self.global_positive_indices), len(self.global_negative_indices))
        
        balanced_positive = self._replicate_to_size(self.global_positive_indices, majority_size)
        balanced_negative = self._replicate_to_size(self.global_negative_indices, majority_size)
        
        # logger.info(f"SMOTE balancing applied: {len(balanced_positive)} positive, {len(balanced_negative)} negative samples")
        return balanced_positive + balanced_negative
    
    @staticmethod
    def _replicate_to_size(indices: List[Tuple[int, int, int]], target_size: int) -> List[Tuple[int, int, int]]:
...
class MultiInstrumentDataGenerator:
    ...
    def __init__(self, config: MultiInstrumentDatasetConfig):
        ...
        self.index_manager = MultiInstrumentIndexManager(self.processors, config)
    
    def get_total_training_samples(self) -> int:
        """Calculate total number of training samples across all instruments"""
        if not self.config.apply_smote:
            return (len(self.index_manager.global_positive_indices) + 
                   len(self.index_manager.global_negative_indices))
        
        if not self.index_manager.global_positive_indices or not self.index_manager.global_negative_indices:
            return (len(self.index_manager.global_positive_indices) + 
                   len(self.index_manager.global_negative_indices))
        
        # Return 2 * majority class size (balanced dataset)
        majority_size = max(
            len(self.index_manager.global_positive_indices),
            len(self.index_manager.global_negative_indices)
        )
    ...
    def generate_batches(self) -> Generator[Tuple[tf.Tensor, tf.Tensor, tf.Tensor], None, None]:
        ...
        while True:  # Infinite generator for epochs
            ...
            for instrument_idx, chunk_idx, row_idx in working_indices:
                try:
                    # Extract sample using the correct instrument processor
    ...
    def get_dataset_info(self) -> Dict[str, Any]:
        ...
        total_positive = sum(stats['positive_samples'] for stats in instrument_stats)
        total_negative = sum(stats['negative_samples'] for stats in instrument_stats)
        ...
        return {
            ...
            'smote_enabled': self.config.apply_smote,
...
def create_multi_instrument_dataset(config: MultiInstrumentDatasetConfig, repeat_dataset: bool = False) -> Tuple[tf.data.Dataset, int]:
    ...
    Args:
        config: Multi-instrument dataset configuration object
        repeat_dataset: Whether to repeat dataset infinitely
    
    Returns:
        Tuple of (TensorFlow dataset, total sample count)
    """
    generator = MultiInstrumentDataGenerator(config)
    
    # Print comprehensive dataset info
    info = generator.get_dataset_info()
    logger.info("=== Multi-Instrument Dataset Info ===")
    logger.info(f"Total instruments: {info['total_instruments']}")
    for instrument_info in info['instruments']:
        logger.info(f"  {instrument_info['name']}: {instrument_info['positive_samples']} pos, "
                   f"{instrument_info['negative_samples']} neg, "
                   f"{instrument_info['total_chunks']} chunks")
    logger.info(f"Global totals: {info['total_positive_samples']} positive, {info['total_negative_samples']} negative")
    logger.info(f"SMOTE: {info['smote_enabled']}, Shuffle: {info['shuffle_enabled']}")
    logger.info("=====================================")
